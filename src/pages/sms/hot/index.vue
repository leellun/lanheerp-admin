...
import type { HomeHotProduct, HomeHotProductSearch, } from '@/api/sms/hotProductApi'
import { _getHomeRecommendProductPage, _updateHotProductSort, _updateHotProductRecommendStatus, _deleteHotProduct } from '@/api/sms/hotProductApi'
import { Form, Modal } from 'ant-design-vue';
import HomeNewProductModal from './modals/HomeHotProductModal.vue'
const modalVisible = ref<boolean>(false)
const pagination = reactive<any>({ pageSize: 10, current: 1 })
const selectedRowKeys = ref<string[]>([]);
const data = ref<Array<HomeHotProduct | any>>([]);
const formRef = reactive<Partial<HomeHotProductSearch>>({
    pageNo: 1,
    pageSize: 10,
...
let sortTimeOut: any = {};
const handleSort = (newProduct: HomeHotProduct, value: number) => {
    ...
    sortTimeOut[newProduct.id] = setTimeout(() => {
        _updateHotProductSort(newProduct.id, value).then(res => {
...
const handleDelete = (row: HomeHotProduct) => {
    _deleteHotProduct([row.id]).then(res => {
        handleSearch()
    });
}
...
const getPageLists = () => {
    ...
    _getHomeRecommendProductPage(formRef as HomeHotProductSearch).then(res => {
...
const handleUpdateStatus = (row: HomeHotProduct) => {
    Modal.confirm({
        title: "提示",
        content: "是否要进行此操作?",
        okText: "确定",
        cancelText: "取消",
        onOk: () => {
            _updateHotProductRecommendStatus([row.id], row.recommendStatus === 1 ? 0 : 1).then(res => {
                getPageLists()
            });
        },
    });
