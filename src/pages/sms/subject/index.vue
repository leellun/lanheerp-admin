<template>
    <div class="main-container">
        ...
        <HomeSubjectModal v-model:visible="modalVisible" v-if="modalVisible" @ok="handleOk" />
    </div>
</template>
<script setup lang="ts"  >
import { ref, reactive } from 'vue'
import type { HomeRecommendSubject, HomeRecommendSubjectSearch, } from '@/api/sms/homeSubjectApi'
import { _getHomeRecommendSubjectPage, _updateHomeSubjectSort, _updateRecommendSubjectStatus, _deleteHomeSubject } from '@/api/sms/homeSubjectApi'
import { Form, Modal } from 'ant-design-vue';
import HomeSubjectModal from './modals/HomeSubjectModal.vue'
const modalVisible = ref<boolean>(false)
const pagination = reactive<any>({ pageSize: 10, current: 1 })
const selectedRowKeys = ref<string[]>([]);
const data = ref<Array<HomeRecommendSubject | any>>([]);
const formRef = reactive<Partial<HomeRecommendSubjectSearch>>({
    pageNo: 1,
    pageSize: 10,
...
const columns = [
    ...
    { title: '专题名称', dataIndex: 'subjectName', key: 'subjectName', width: 150, align: 'center' },
...
let sortTimeOut: any = {};
const handleSort = (newSubject: HomeRecommendSubject, value: number) => {
    newSubject.sort = value
    if (sortTimeOut[newSubject.id] !== undefined) {
        clearTimeout(sortTimeOut[newSubject.id])
    }
    sortTimeOut[newSubject.id] = setTimeout(() => {
        _updateHomeSubjectSort(newSubject.id, value).then(res => {
...
const handleDelete = (row: HomeRecommendSubject) => {
    _deleteHomeSubject([row.id]).then(res => {
        handleSearch()
    });
}
...
const getPageLists = () => {
    ...
    _getHomeRecommendSubjectPage(formRef as HomeRecommendSubjectSearch).then(res => {
...
const handleUpdateStatus = (row: HomeRecommendSubject) => {
    Modal.confirm({
        title: "提示",
        content: "是否要进行此操作?",
        okText: "确定",
        cancelText: "取消",
        onOk: () => {
            _updateRecommendSubjectStatus([row.id], row.recommendStatus === 1 ? 0 : 1).then(res => {
                getPageLists()
            });
        },
    });
